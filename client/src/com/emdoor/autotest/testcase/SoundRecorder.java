...
public class SoundRecorder extends Activity implements Button.OnClickListener,
	...
	public void onCreate(Bundle icycle) {
		...
		isExtMic = getIntent().getBooleanExtra("ext_mic", false);
		Log.d(TAG, "isExtMic:" + isExtMic);
		...
		pass = (Button) findViewById(R.id.btnrecorderok);
		fail = (Button) findViewById(R.id.btnrecorderfail);
		back = (Button) findViewById(R.id.btnrecorderback);
	...
	private final Button.OnClickListener listener = new Button.OnClickListener() {
		public void onClick(View args0) {
			...
			switch (args0.getId()) {
			case R.id.btnrecorderok:
				if (isExtMic) {
					config.setExtMICTestOk(1);
				} else {
					config.setMICTestOk(1);
				}
				break;
			case R.id.btnrecorderfail:
				if (isExtMic) {
					config.setExtMICTestOk(2);
				} else {
					config.setMICTestOk(2);
				}
				break;
			default:
				if (isExtMic) {
					config.setExtMICTestOk(0);
				} else {
					config.setMICTestOk(0);
				}
				break;
			}
	...
	private Uri createPlaylist(Resources res, ContentResolver resolver) {
		...
		if (uri == null) {
			new AlertDialog.Builder(this).setTitle(R.string.app_name_selftest)
	...
	private void showDialog() {
		new AlertDialog.Builder(this)
				.setTitle("Plug External Mic")
				.setMessage(
						"Plug external microphone and click OK to start test.")
				.setPositiveButton(R.string.button_ok, null)
				.setCancelable(false).show();
	}
